...
```{r}
census_get<-function(year=2022,numvar="AGEP",catvar="SEX",geo_level="state",region_select="01") {
  #Building the URL
  #"https://api.census.gov/data/2022/acs/acs1/pums?get=PWGTP,SCHL&for=state:01"
  ...
  if (geo_level=="All") {
    ...
    URL<-paste0("https://api.census.gov/data/",year,"/acs/acs1/pums?get=PWGTP,",paste(numvar,collapse=","),",",paste(catvar,collapse=","),"&for=",geo_level,":",region_select)
  }
  ...
  #num_cols<-!(numvar %in% c("JWAP","JWDP"))
  
  #if (length(num_cols)>1) {
  #  num_var_string<-paste(num_cols,collapse=",")
  #  clean_response<- clean_response |>
  #    mutate(across(noquote(num_var_string)),as.numeric())
  #} else if (length(num_cols==1)) {
  #  clean_response<- clean_response |>
  #    mutate(across(noquote(numvar)),as.numeric())
  #} 
...
test_response<-census_get(catvar=c("SEX","HHL"))


test<-read_json(paste0("https://api.census.gov/data/",year,"/acs/acs1/pums/variables/JWAP.json"))

unlist(test$values$item,use.names=FALSE)
unlist(names(test$values$item))
```
